...
const int SIZE = 1e6; // 짝수인 정수의 최대값인 10^6을 표현하는 방법
...
vector<bool> isPrime() {
    ...
    for (int i = 2; i * i <= SIZE; i++) { //i가 루트SIZE일때까지의 값을 반복
        if (!is_prime[i]) { //소수가 아니라면
            continue;//빠져나옴
        }
        //i가 소수라면
        for (int j = i * i; j <= SIZE; j += i) {// j=j+i , j는 i*i에서 SIZE까지 i씩 더해서 반복
            is_prime[j] = false; //i의 배수 제거
        }
    }
    return is_prime; //is_prime 소수인 값 반환
}
