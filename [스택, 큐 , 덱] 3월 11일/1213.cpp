//
// Created by LEE YUN JIN on 2022-03-20.
//
...
using namespace std;// std 이름공간을 사용

const int SIZE = 26;    //알파벳 개수

vector<int> countAlphabet(string text) {
    // 텍스트에 있는 각 알파벳의 개수를 세어 count를 리턴하는 함수 생성
    vector<int> count(SIZE, 0); //A~Z 까지의 문자 개수를 저장할 리스트
    for (int i = 0; i < text.size(); i++) {
        // 각 문자를 인덱스로 바꿔주기 위해 'A'의 인덱스를 빼줌
        count[text[i] - 'A']++;
    }
    return count; //알파벳의 개수를 반환
}

string makePalindrome(vector<int> &count) {
    // 팰린드롬을 만들어 리턴하는 함수.
    string part1 = "", part2 = "", part3 = ""; //펠린드롬을 구성하는 3파트 초기화
    ...
    for (int i = 0; i < SIZE; i++) {
        ...
        //알파벳 수가 짝수일 때 //남은 수의 절반을 나누어 part1과 part3에 붙인다.
    ...
    return part1 + part2 + part3;//문자열끼리 더한 값을 반환
}
